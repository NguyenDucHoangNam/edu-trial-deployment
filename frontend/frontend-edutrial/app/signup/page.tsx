...
const SignupPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (
        response.message?.includes('Tài khoản đã được đăng ký nhưng chưa xác thực')
      ) {
        showInfo(
          'Tài khoản đã tồn tại nhưng chưa xác thực. Vui lòng kiểm tra email của bạn để lấy mã OTP hoặc nhấn Gửi Lại Mã OTP'
        );
      } else {
      ...
      router.push(`/verify-otp?email=${encodeURIComponent(formData.email)}&firstTime=false`);
    } catch (error: any) {
      const errorCode = error?.response?.data?.errorCode;
      const message = error?.response?.data?.message ?? 'Đã xảy ra lỗi không xác định.';

      if (errorCode === 'USER_NOT_VERIFIED') {
        showInfo('Tài khoản đã tồn tại nhưng chưa xác thực. Vui lòng kiểm tra email hoặc nhấn Gửi Lại Mã OTP');
        router.push(`/verify-otp?email=${encodeURIComponent(formData.email)}&firstTime=false`);
      } else if (errorCode === 'USER_EXISTED') {
        showError('Tài khoản đã tồn tại và đã được xác thực. Vui lòng đăng nhập.');
      } else {
        showError('Đăng ký thất bại! Vui lòng thử lại sau.');
        console.error('[Đăng ký thất bại]', message);
      }
    }
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  <div className="mb-8 flex">
                    ...
                    >
                      ...
                      <span>
                        ...
                        <a href="#0" className="text-primary hover:underline"> Điều khoản và điều kiện </a>, và
                        <a href="#0" className="text-primary hover:underline"> Chính sách bảo mật </a>
                      </span>
                    </label>
                  </div>
                  ...
                  <div className="mb-6">
                    <button
                      ...
                      className={`shadow-submit dark:shadow-submit-dark flex w-full items-center justify-center rounded-sm px-9 py-4 text-base font-medium text-white duration-300 ${acceptedTerms ? 'bg-primary hover:bg-primary/90' : 'bg-gray-400 cursor-not-allowed'
                        }`}
                    >
                      Đăng ký
                    </button>
